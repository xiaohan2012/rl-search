...
class FmimGenerationTest(unittest.TestCase):
    ...
    def test_feature_matrix(self):
        self.assertEqual(self.fmim._kw2doc_m.shape, (8, 10))
        self.assertEqual(self.fmim._doc2kw_m.shape, (10, 8))
        
    def test_index_mapping(self):
        kws = [u'a', u'database', u'mysql', u'python', u'redis', u'the', u'tornado', u'web']
        for ind, kw in enumerate(kws):
            self.assertEqual(self.fmim._kw_ind[kw], ind)
            
        for doc_id in xrange(1, 11):
            self.assertEqual(self.fmim._doc_ind[doc_id], doc_id - 1)

    def test_inverse_index_mapping(self):
        kws = [u'a', u'database', u'mysql', u'python', u'redis', u'the', u'tornado', u'web']
        for ind, kw in enumerate(kws):
            self.assertEqual(self.fmim._kw_ind_r[ind], kw)
            
        for doc_id in xrange(1, 11):
            self.assertEqual(self.fmim._doc_ind_r[doc_id-1], doc_id)
